 ...
 *   DELETE /api/categorias/:nombre    -> eliminar categoría
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (catMatch && method === 'DELETE') {
        return await deleteCategoria(decodeURIComponent(catMatch[1]), env);
      }
...
async function deleteCategoria(nombre, env) {
  const result = await env.DB.prepare('DELETE FROM categorias WHERE nombre = ?')
    .bind(nombre).run();
